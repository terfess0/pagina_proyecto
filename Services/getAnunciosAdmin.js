import { getAnuncios, updateAnuncio, deleteAnuncio } from "../Services/firebase.js";
...
async function viewAnuncios() {
    try {
        ...
        const caja = document.getElementById('contain_admin_anuncios');
        caja.innerHTML = '';
...
async function save(id, row) {
    ...
    try {
        await updateAnuncio(id, newData);
...
async function delete_data(docUserId) {
    ...
    try {
        const action = deleteAnuncio(docUserId)
